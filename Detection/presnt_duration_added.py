        ...
        with self.lock:
            ...
            for sn in ALL_SESSIONS:
                ...
                for row in data:
                    ...
                    prn = doc.get('PRN No.') or doc.get('PRN No') or doc.get('PRN_No') or ''
                    if not prn:
                        continue
                    students[prn] = {
                        'prn_no': prn,
                        'roll_no': doc.get('Roll No', ''),
                        'name': doc.get('Name', ''),
                        'status': 'Absent',
                        'timestamps': {'first_seen': None, 'last_seen': None, 'present_timer_start': None,
                                      'absence_timer_start': None, 'last_updated': ct},
                        'durations': {'total_present_seconds': 0, 'total_absent_seconds': 0,
                                     'total_present_human': '0 sec', 'total_absent_human': '0 sec'},
                        'flags': {'manual_override': False, 'is_temp_absent': False, 'is_perm_absent': False}
                    }
                sessions[sn] = {'start_time': None, 'end_time': None, 'students': students}
            col.insert_one({
            ...
            self.db.lecture_metadata.insert_one({
                ...
                'year_code': yc, 'classroom': room, 'teacher_name': teacher, 'created_at': datetime.now()
            })
            return cn
...
class AttendanceSystem:
    ...
    def mark_attendance(self, ident):
        ...
        if not prn:
            return False
        if prn in self.student_status:
            self.student_status[prn]['last_seen'] = ct
            self.student_status[prn]['status'] = 'Present'
            self.student_status[prn]['timer_start'] = None
        else:
            self.student_status[prn] = {'last_seen': ct, 'status': 'Present', 'timer_start': None}
            self.attendance_count += 1
        return self.db.update_student_attendance(self.current_collection, self.current_session, 
                                                  prn, 'Present', manual=False)
    ...
    def process_frame(self, frame):
        try:
            ...
            cv2.putText(frame, f"Session: {sess}", (20, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            cv2.putText(frame, f"Attendance: {self.attendance_count}/{self.total_students}", 
                       (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0) if self.attendance_count > 0 else (255, 255, 255), 2)
...
def preview_config():
    try:
        ...
        sheet = f"{dept}_{db._get_year_code(y)}"
        try:
            h, data = db.load_students_from_excel(TEMPLATE_FILE, sheet)
        except Exception as e:
            db.close()
            return jsonify({'success': False, 'message': f'Sheet not found: {sheet}', 'error': str(e)}), 400
        sess = get_current_session()
        date = datetime.now().strftime('%Y-%m-%d')
        ...
        db.close()
        return jsonify({'success': True, 'year_code': db._get_year_code(y), 'sheet_loaded': sheet,
                       'collection_name': cn, 'session_name': sess, 'date': date, 'summary': summ, 'attendance': att})
    except Exception as e:
...
def start_camera():
    ...
    try:
        ...
        attendance_system = AttendanceSystem(d.get('mode', 1), d['year'], d['department'], 
        ...
        sess = get_current_session()
        date = datetime.now().strftime('%Y-%m-%d')
        cn = attendance_system.db.create_or_get_daily_collection(d['department'], d['year'], date, 
        ...
        return jsonify({'success': True, 'message': 'Camera started', 
                       ...
                       'initial_data': {'collection_name': cn, 'session_name': sess, 'date': date,
    ...
    try:
        if attendance_system and attendance_system.current_collection and attendance_system.current_session:
            return jsonify({'success': True, 'active': True,
                          ...
                          'summary': attendance_system.db.get_session_summary(attendance_system.current_collection, 
                                                                              attendance_system.current_session),
                          'attendance': attendance_system.db.get_session_attendance(attendance_system.current_collection, 
                                                                                   attendance_system.current_session)})
...
def update_attendance_manual():
    try:
        d = request.get_json()
        db = DatabaseManager(MONGODB_CONFIG)
        success = db.update_student_attendance(d['collection_name'], d['session_name'], 
                                               d['prn_no'], d['status'], manual=True)
        db.close()
        return jsonify({'success': True, 'message': f'Marked as {d["status"]}'}) if success else \
               jsonify({'success': False, 'error': 'Failed to update'}), 500
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
<div class="camera-section"><div class="error-msg" id="errorBanner"></div><div class="config-row">
...
updateStats(data.summary);displayData(data.attendance);document.getElementById('videoContainer').innerHTML='<div class="video-placeholder">Ready to start camera</div>'}
catch(e){showError('Network error: '+e.message)}finally{btn.disabled=false;btn.textContent='Confirm'}}
...
if(autoDetected)row.classList.add('row-auto-detected');row.innerHTML=`<td>${r.prn_no||'-'}</td><td>${r.roll_no||'-'}</td><td>${r.name||'-'}</td>
...
<td><button class="status-toggle ${st==='Present'?'to-absent':'to-present'}" onclick="toggleAtt('${r.prn_no}','${st}')">${st==='Present'?'Mark Absent':'Mark Present'}</button></td>`;
tb.appendChild(row)})}async function toggleAtt(prn,cur){if(!col||!sess){showError('No active session');return}const ns=cur==='Present'?'Absent':'Present',
btn=event.target,origText=btn.textContent;btn.disabled=true;btn.textContent='Updating...';try{const r=await fetch(`${API}/attendance/update`,
...
const d=await r.json();if(d.success)await refreshData();else showError(d.error||'Failed to update')}catch(e){showError('Network error: '+e.message)}
finally{btn.disabled=false;btn.textContent=origText}}async function refreshData(){await loadSession()}
...
if __name__=='__main__':
    print("="*80)
    print("ATTENDANCE SYSTEM - COMPLETE VERSION")
    print("="*80)
    print("\n✅ ALL FEATURES IMPLEMENTED:")
    print("   1. PRN-based attendance tracking")
    print("   2. Automatic face recognition with auto-detection badges")
    print("   3. Manual override with manual badges")
